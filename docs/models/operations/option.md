...
```go
cipherswarmagentsdkgo.WithServerURL("https://api.example.com")
```
...
```go
cipherswarmagentsdkgo.WithTemplatedServerURL("https://{host}:{port}", map[string]string{
...
```go
cipherswarmagentsdkgo.WithDefaultHost(/* ... */)
```
...
```go
cipherswarmagentsdkgo.WithHostAddress(/* ... */)
```
...
```go
cipherswarmagentsdkgo.WithHostPort(/* ... */)
```
...
```go
cipherswarmagentsdkgo.WithClient(httpClient)
```
...
```go
cipherswarmagentsdkgo.WithSecurity(/* ... */)
```
...
```go
cipherswarmagentsdkgo.WithSecuritySource(/* ... */)
```
...
```go
cipherswarmagentsdkgo.WithRetryConfig(retry.Config{
    Strategy: "backoff",
...
```go
cipherswarmagentsdkgo.WithTimeout(30 * time.Second)
```
